...
const (
	READ_LEN = Dm_build_1094
)

type iOffRowBinder interface {
	read(buf *Dm_build_282)
	isReadOver() bool
	getObj() interface{}
}
...
func newOffRowBytesBinder(obj []byte, encoding string) *offRowBytesBinder {
	...
	binder.offRow = binder.buffer.Dm_build_287() > Dm_build_1091
	return binder
}

func (b *offRowBytesBinder) read(buf *Dm_build_282) {
	if b.buffer.Dm_build_287() > 0 {
		buf.Dm_build_319(b.buffer)
	} else if !b.readOver {
		var obj = b.obj.([]byte)
...
func (b *offRowBlobBinder) read(buf *Dm_build_282) {
	if b.buffer.Dm_build_287() > 0 {
		buf.Dm_build_319(b.buffer)
	} else if !b.readOver {
...
func (b *offRowClobBinder) read(buf *Dm_build_282) {
	if b.buffer.Dm_build_287() > 0 {
		buf.Dm_build_319(b.buffer)
	} else if !b.readOver {
...
func (b *offRowReaderBinder) read(buf *Dm_build_282) {
	if b.buffer.Dm_build_287() > 0 {
		buf.Dm_build_319(b.buffer)
	} else if !b.readOver {
		var err error
		...
		buf.Dm_build_308(bytes[0:readLen], 0, readLen)
	}
}

func (b *offRowReaderBinder) readAll() []byte {
	var byteArray = Dm_build_286()
	b.read(byteArray)
	for !b.readOver {
		b.read(byteArray)
	}
	return byteArray.Dm_build_329()
}
