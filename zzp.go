...
func (typeDescriptor *TypeDescriptor) parseDescByName() error {
	...
	err = typeDescriptor.unpack(Dm_build_365(buf))
	if err != nil {
		return err
	}
	return nil
...
func pack(typeDesc *TypeDescriptor, msg *Dm_build_360) error {
	switch typeDesc.column.colType {
	case ARRAY, SARRAY:
	...
	msg.Dm_build_415(typeDesc.column.colType)

	msg.Dm_build_415(typeDesc.column.prec)

	msg.Dm_build_415(typeDesc.column.scale)
	return nil
}
...
func packArray(arrDesc *TypeDescriptor, msg *Dm_build_360) error {

	msg.Dm_build_415(arrDesc.column.colType)

	msg.Dm_build_471(arrDesc.m_sqlName.m_name, arrDesc.getServerEncoding(), arrDesc.m_conn)

	msg.Dm_build_415(int32(arrDesc.m_objId))

	msg.Dm_build_415(int32(arrDesc.m_objVersion))

	msg.Dm_build_415(int32(arrDesc.m_length))

	return pack(arrDesc.m_arrObj, msg)
}

func packRecord(strctDesc *TypeDescriptor, msg *Dm_build_360) error {

	msg.Dm_build_415(strctDesc.column.colType)

	msg.Dm_build_471(strctDesc.m_sqlName.m_name, strctDesc.getServerEncoding(), strctDesc.m_conn)

	msg.Dm_build_415(int32(strctDesc.m_objId))

	msg.Dm_build_415(int32(strctDesc.m_objVersion))

	msg.Dm_build_411(int16(strctDesc.m_size))
...
func packClass(strctDesc *TypeDescriptor, msg *Dm_build_360) error {
	...
	if strctDesc.m_objId == 4 {
		...
		msg.Dm_build_415(int32(strctDesc.m_outerId))

		msg.Dm_build_415(int32(strctDesc.m_outerVer))

		msg.Dm_build_415(int32(strctDesc.m_subId))
...
func (typeDescriptor *TypeDescriptor) unpack(buffer *Dm_build_360) error {

	typeDescriptor.column.colType = buffer.Dm_build_489()
	...
	typeDescriptor.column.prec = buffer.Dm_build_489()

	typeDescriptor.column.scale = buffer.Dm_build_489()
	return nil
}

func (typeDescriptor *TypeDescriptor) unpackArray(buffer *Dm_build_360) error {

	typeDescriptor.m_sqlName.m_name = buffer.Dm_build_539(typeDescriptor.getServerEncoding(), typeDescriptor.m_conn)

	typeDescriptor.m_sqlName.m_schId = int(buffer.Dm_build_489())

	typeDescriptor.m_sqlName.m_packId = int(buffer.Dm_build_489())

	typeDescriptor.m_objId = int(buffer.Dm_build_489())

	typeDescriptor.m_objVersion = int(buffer.Dm_build_489())

	typeDescriptor.m_length = int(buffer.Dm_build_489())
...
func (typeDescriptor *TypeDescriptor) unpackRecord(buffer *Dm_build_360) error {
	...
	typeDescriptor.m_size = int(buffer.Dm_build_504())
...
func (typeDescriptor *TypeDescriptor) unpackClnt_nestTab(buffer *Dm_build_360) error {

	typeDescriptor.m_maxCnt = int(buffer.Dm_build_489())
...
func (typeDescriptor *TypeDescriptor) unpackClnt(buffer *Dm_build_360) error {

	typeDescriptor.m_outerId = int(buffer.Dm_build_489())

	typeDescriptor.m_outerVer = int(buffer.Dm_build_489())

	typeDescriptor.m_subId = int(buffer.Dm_build_504())

	typeDescriptor.m_cltnType = int(buffer.Dm_build_504())
...
func (typeDescriptor *TypeDescriptor) unpackClass(buffer *Dm_build_360) error {
